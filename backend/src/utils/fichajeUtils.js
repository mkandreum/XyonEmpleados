const { getScheduleForDay, detectTurno, timeToMinutes, DAY_NAMES_ES } = require('../services/smartScheduleService');
...
function groupFichajesByDay(fichajes, schedule) {
    ...
    return Array.from(groups.entries()).map(([date, dayFichajes]) => {
        ...
        if (Array.isArray(schedule)) {
            // schedule is a shifts array
            const firstFichaje = sorted.find(f => f.tipo === 'ENTRADA');
            const refDate = firstFichaje ? new Date(firstFichaje.timestamp) : new Date(date + 'T12:00:00');
            const resolvedShift = selectClosestShift(schedule, refDate);
            if (resolvedShift) {
                // Check if there are overrides in resolvedShift.scheduleOverrides for this date
                let dayOverride = null;
                if (resolvedShift.scheduleOverrides && typeof resolvedShift.scheduleOverrides === 'object') {
                    dayOverride = resolvedShift.scheduleOverrides[date];
                }
                ...
                if (dayOverride && typeof dayOverride === 'object') {
                    if (dayOverride.dayOff === true) {
                        ...
                        daySchedule = {
                            horaEntrada: dayOverride.horaEntrada || resolvedShift.horaEntrada,
                            horaSalida: dayOverride.horaSalida || resolvedShift.horaSalida,
                            horaEntradaTarde: dayOverride.horaEntradaTarde !== undefined ? dayOverride.horaEntradaTarde : (resolvedShift.horaEntradaTarde || null),
                            horaSalidaMañana: dayOverride.horaSalidaMañana !== undefined ? dayOverride.horaSalidaMañana : (resolvedShift.horaSalidaMañana || null),
                            toleranciaMinutos: dayOverride.toleranciaMinutos !== undefined ? dayOverride.toleranciaMinutos : resolvedShift.toleranciaMinutos,
                            flexibleSchedule: dayOverride.flexibleSchedule !== undefined ? dayOverride.flexibleSchedule : resolvedShift.flexibleSchedule,
                            isOverride: true,
                            dayName: DAY_NAMES_ES[refDate.getDay()]
                        };
                    }
                } else {
                    daySchedule = {
                        horaEntrada: resolvedShift.horaEntrada,
                        horaSalida: resolvedShift.horaSalida,
                        horaEntradaTarde: resolvedShift.horaEntradaTarde || null,
                        horaSalidaMañana: resolvedShift.horaSalidaMañana || null,
                        toleranciaMinutos: resolvedShift.toleranciaMinutos,
                        flexibleSchedule: resolvedShift.flexibleSchedule,
                        isOverride: false,
                        dayName: DAY_NAMES_ES[refDate.getDay()]
                    };
                }
            ...
            daySchedule = getScheduleForDay(schedule, dateObj);
        }
