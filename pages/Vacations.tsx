...
import { mockVacations } from '../services/mockData';
...
export const VacationsPage: React.FC = () => {
  ...
  const pendingDays = mockVacations
    .filter(v => v.status === VacationStatus.PENDING && v.type === 'VACATION')
    .reduce((acc, curr) => acc + curr.days, 0);
  const remainingDays = totalDays - takenDays - pendingDays;

  const data = [
    { name: 'Disfrutados', value: takenDays, color: '#94a3b8' }, // Slate-400
    { name: 'Pendientes', value: pendingDays, color: '#f59e0b' }, // Amber-500
    { name: 'Disponibles', value: remainingDays, color: '#2563eb' }, // Blue-600
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 lg:col-span-1 flex flex-col items-center justify-center">
            <h2 className="text-lg font-semibold text-slate-900 mb-4 w-full">Balance Anual 2024</h2>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-100 lg:col-span-2 overflow-hidden flex flex-col">
            ...
            {showRequestForm && (
                <div className="p-6 bg-blue-50 border-b border-blue-100">
                    ...
                    <form className="grid grid-cols-1 md:grid-cols-2 gap-4" onSubmit={(e) => { e.preventDefault(); alert("Solicitud enviada (Simulación)"); setShowRequestForm(false); }}>
                        <div>
                            <label className="block text-sm font-medium text-slate-700 mb-1">Fecha Inicio</label>
                            <input type="date" className="w-full border border-slate-300 rounded-lg p-2 focus:ring-2 focus:ring-blue-500 focus:outline-none" required />
                        </div>
                         <div>
                        ...
                        <div>
                            ...
                            <select className="w-full border border-slate-300 rounded-lg p-2 focus:ring-2 focus:ring-blue-500 focus:outline-none">
                        ...
                        <div className="md:col-span-2 flex justify-end gap-3 mt-2">
                            ...
                            <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Enviar Solicitud</button>
                        </div>
                    </form>
                </div>
            )}
                            ...
                            <tr key={vac.id} className="hover:bg-slate-50">
                                <td className="px-6 py-4">
                                    <div className="flex flex-col">
                                        <span className="font-medium text-slate-900">{vac.startDate}</span>
                                        <span className="text-slate-500 text-xs">hasta {vac.endDate}</span>
                                    </div>
                                </td>
