...
export const fichajeService = {
    create: async (tipo: FichajeTipo) => {
        const response = await api.post<{ fichaje: Fichaje; status: FichajeStatus; turno?: TurnoInfo | null; assignedShift?: any; warning?: string | null; gpsValidation?: any }>('/fichajes', { tipo });
        return response.data;
    },
    getCurrent: async () => {
